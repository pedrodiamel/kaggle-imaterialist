...
def load_image(path):
    ...
    if is_url(path):
        ...
        try:
            ...
            print('IOError: Trying to load "%s": %s' % (path, e.message))
            raise errors.LoadImageError
    else:
    ...
    if image.mode in ['L', 'RGB']:
        ...
        print('Image mode "%s" not supported'.format(image.mode) )
        raise errors.LoadImageError
...
def image_to_array(image,
    ...
    if channels not in [None, 1, 3, 4]:
        raise ValueError('unsupported number of channels: %s'.format(channels) ) 
    ...
    if isinstance(image, PIL.Image.Image):
        ...
        if channels is None:
            ...
            if image_mode not in ['L', 'RGB', 'RGBA']:
                raise ValueError('unknown image mode "%s"'.format(image_mode) )
        elif channels == 1:
        ...
        if channels is None:
            if not (image.ndim == 2 or (image.ndim == 3 and image.shape[2] in [3, 4])):
                raise ValueError('invalid image shape: %s'.format(image.shape,) )
        elif channels == 1:
            if image.ndim != 2:
...
def get_color_map(name):
    ...
    if name == 'white':
        ...
        if name != 'jet':
            print('Warning: colormap "%s" not supported. Using jet instead.'.format( name ))
